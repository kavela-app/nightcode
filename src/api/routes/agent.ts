...
- list_repos: {}
- create_task: { "repo_id": number, "title": string, "prompt": string, "workflow"?: "implement-pr"|"plan-implement-pr"|"plan-audit-implement-pr", "priority"?: 1-10 }
- run_task: { "task_id": number }
- pause_task: { "task_id": number }
- cancel_task: { "task_id": number }
- list_tasks: { "status"?: string }
- get_stats: {}
- create_schedule: { "name": string, "cron_expr": string, "repo_id": number, "title": string, "prompt": string, "workflow"?: string, "priority"?: number }
...
async function executeAction(
  ...
  switch (action) {
    ...
    case "create_schedule": {
      ...
      const repo = db.select().from(schema.repos).where(eq(schema.repos.id, repoId)).get();
      if (!repo) {
        return { reply: `Repo id=${repoId} not found.`, action: "create_schedule", data: { error: "repo_not_found" } };
      }
      const taskTemplate = {
        repoId,
        title: params.title as string,
      ...
      const result = db
        ...
        .values({
          name: params.name as string,
          cronExpr: params.cron_expr as string,
          taskTemplate: JSON.stringify(taskTemplate),
        })
        .returning()
        .get();
      return { reply: `Schedule "${result.name}" created (id=${result.id}) with cron "${result.cronExpr}".`, action: "create_schedule", data: result };
    }
